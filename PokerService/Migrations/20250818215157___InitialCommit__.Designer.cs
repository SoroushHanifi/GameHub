// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using PokerService.Data;

#nullable disable

namespace PokerService.Migrations
{
    [DbContext(typeof(PokerDbContext))]
    [Migration("20250818215157___InitialCommit__")]
    partial class __InitialCommit__
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("PokerService.Models.Card", b =>
                {
                    b.Property<int>("Suit")
                        .HasColumnType("int");

                    b.Property<int>("Rank")
                        .HasColumnType("int");

                    b.Property<Guid?>("GameStateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Suit", "Rank");

                    b.HasIndex("GameStateId");

                    b.HasIndex("PlayerId");

                    b.ToTable("Card");
                });

            modelBuilder.Entity("PokerService.Models.GameState", b =>
                {
                    b.Property<Guid>("GameStateId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CurrentBet")
                        .HasColumnType("int");

                    b.Property<string>("CurrentTurnUsername")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("GameStateId");

                    b.ToTable("GameState");
                });

            modelBuilder.Entity("PokerService.Models.Player", b =>
                {
                    b.Property<Guid>("PlayerId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Chips")
                        .HasColumnType("int");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("PlayerId");

                    b.HasIndex("RoomId");

                    b.ToTable("Player");
                });

            modelBuilder.Entity("PokerService.Models.Room", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("GameStateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDelete")
                        .HasColumnType("bit");

                    b.Property<int>("Pot")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("GameStateId");

                    b.ToTable("Rooms");
                });

            modelBuilder.Entity("PokerService.Models.Card", b =>
                {
                    b.HasOne("PokerService.Models.GameState", null)
                        .WithMany("CommunityCards")
                        .HasForeignKey("GameStateId");

                    b.HasOne("PokerService.Models.Player", null)
                        .WithMany("Hand")
                        .HasForeignKey("PlayerId");
                });

            modelBuilder.Entity("PokerService.Models.Player", b =>
                {
                    b.HasOne("PokerService.Models.Room", null)
                        .WithMany("Players")
                        .HasForeignKey("RoomId");
                });

            modelBuilder.Entity("PokerService.Models.Room", b =>
                {
                    b.HasOne("PokerService.Models.GameState", "GameState")
                        .WithMany()
                        .HasForeignKey("GameStateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("GameState");
                });

            modelBuilder.Entity("PokerService.Models.GameState", b =>
                {
                    b.Navigation("CommunityCards");
                });

            modelBuilder.Entity("PokerService.Models.Player", b =>
                {
                    b.Navigation("Hand");
                });

            modelBuilder.Entity("PokerService.Models.Room", b =>
                {
                    b.Navigation("Players");
                });
#pragma warning restore 612, 618
        }
    }
}
